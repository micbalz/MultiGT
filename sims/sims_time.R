...
duration = function(n, p, w1, w2, w3, w4, sims = 0) {
  # Optimization and simulation of expected duration per member in multi-stage (r,s)-regular design algorithms
  # INPUTS:
  # n: Integer. Size of the population
  # p: Numeric (0 ≤ p ≤ 1). Probability of defectiveness
  # w1,w2,w3,w4: Numeric. Units of time at each stage
  ...
  #     results$Theoretical = Expected duration per member
  #     results$Tests = Average duration per member
  #     results$Lower = Minimum duration per member
  #     results$Upper = Maximum duration per member
...
### Set population size, time units and probabilities
n = 1000
p_val = seq(0,0.35,0.001)
w1 = w2 = w3 = w4 = 1

time = pblapply(p_val, function(p) duration(n, p, w1, w2, w3, w4, sims = 100))

### Combine list of data frames for each p into a single data frame
results = do.call(rbind, time)
...
simulation$Diff = 100*abs((simulation$Theoretical - simulation$Duration)/ simulation$Theoretical)
...
MAPE = simulation %>%
  mutate(
    Category = factor(case_when(
      ...
      p >= 0.182 ~ "High"
    ), levels = c("Low", "Medium", "High")) # Set the factor levels
  ) %>%
  group_by(Algorithm, Category) %>%
  summarise(MAPE = mean(Diff, na.rm = TRUE), .groups = "drop") %>%
  arrange(Algorithm, Category)  # Ensure sorting by Algorithm and Category

MAPE

### Plot results for the simulation study
ggplot(simulation, aes(x = p)) +
  geom_line(aes(y = Duration, linetype = "Average", color = "Average"), linewidth = 1) +
  ...
  labs(x = "Probability", y = "Average/Expected Duration per Member", 
